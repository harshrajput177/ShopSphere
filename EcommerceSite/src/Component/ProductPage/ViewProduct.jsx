...
import { FiMessageCircle, FiShare2, FiShoppingBag, FiLock } from "react-icons/fi";
...
const ProductPage = ({ product, setProduct }) => {
  ...
  useEffect(() => {
    if (!product) return;
    let recentProducts = JSON.parse(localStorage.getItem("recentViewed")) || [];
    recentProducts = recentProducts.filter((item) => item._id !== product._id);
    recentProducts.unshift(product);
    recentProducts = recentProducts.slice(0, 10);
    localStorage.setItem("recentViewed", JSON.stringify(recentProducts));
  }, [product]);
  ...
  const handleWishlistClick = () => {
    if (!user) {
      showToast({ type: "error", title: "Please login first!", message: "Sign in to continue shopping", icon: "lock" });
      navigate("?auth=login");
      return;
    }
    if (isWishlisted) {
      ...
      showToast({ type: "wishlist", title: "Saved to Wishlist!", message: product.title, icon: "heart" });
    }
  };
  ...
  const handleAddToCart = () => {
    ...
    showToast({ type: "success", title: "Added to Bag!", message: `${product.title} — Size ${selectedSize.size}`, icon: "bag" });
  };
    ...
    <div className="ViewProduct-product-container">
      ...
      <div className="ViewProduct-details-section">
            ...
            {product.variants?.map((variant, i) => (
              <img
                ...
                onClick={() => {
                  ...
                  setActiveImage(variant.mainImage || variant.images?.[0]);
                }}
        ...
        <div className="product-btn-group">
          ...
          <button type="button" className="ViewProduct-cart-btn" onClick={handleAddToCart}>
            <FiShoppingBag />
            ...
            {editing ? (
              <>
                ...
                <button className="pincode-check-btn" onClick={checkPincode}>
                  {pincodeLoading ? <span className="pincode-spinner" /> : "Check"}
                </button>
              </>
            ) : (
